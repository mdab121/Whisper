...
public func whisper(_ message: Message, to: UINavigationController, action: WhisperAction = .show) {
...
class WhisperFactory: NSObject {
  ...
  func moveControllerViews(_ down: Bool) {
    guard let navigationController = self.navigationController,
        let visibleController = navigationController.visibleViewController, Config.modifyInset
      else { return }
  ...
  func performControllerMove(_ viewController: UIViewController) {
    ...
    if let tableView = viewController.view as? UITableView, viewController is UITableViewController {
    ...
    } else if let collectionView = viewController.view as? UICollectionView, viewController is UICollectionViewController {
